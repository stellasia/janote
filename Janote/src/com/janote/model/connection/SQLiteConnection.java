...
public class SQLiteConnection {
	 ...
	 * 
	 * @param DBPath (String)
	 * 			the path to the current database file on the disk (if DBPath="", changed to default value "data/test.sql")
	...
	public static Connection getInstance(String DBPath) {
		...
		*/
		...
		if(connect == null){ // instantiate the class if and only if it was not already done
			...
			try {
				connect = DriverManager.getConnection("jdbc:sqlite:" + path_to_db);
			}
			catch (Exception ez) {
